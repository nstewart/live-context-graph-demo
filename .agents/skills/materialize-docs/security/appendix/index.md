...
| <a href="/sql/alter-materialized-view" ><code>ALTER MATERIALIZED VIEW</code></a> | <ul> <li> <p>Ownership of the materialized view.</p> </li> <li> <p>In addition, to change owners:</p> <ul> <li>Role membership in <code>new_owner</code>.</li> <li><code>CREATE</code> privileges on the containing schema if the materialized view is namespaced by a schema.</li> </ul> </li> <li> <p>In addition, to apply a replacement:</p> <ul> <li>Ownership of the replacement materialized view.</li> </ul> </li> </ul>  |
